<template>
  <view :class="Object.keys(travelTicketData).length > 0 && travelTicketData.status > 0 ? 'ibgw' : 'ibg'">
    ...
    <view class="head">
      <block v-if="travelTicketData.status === 1">
        <image src="/static/images/apply/quan2.jpg" mode="widthFix" />
        <view class="no">{{ travelTicketData.busiParam.no }}</view>
      </block>
      <image v-else src="/static/images/apply/title.jpg" mode="widthFix" />
    </view>
        ...
        <view class="travelBd">
          <view class="steps">
            ...
            <view class="item">
              <view class="left">
                ...
                <view class="value">哈尔滨会展中心体育馆</view>
              </view>
          ...
          <view class="dataInfo">
            ...
            <view v-if="travelTicketData.status > 0" class="item">
              ...
              <view v-if="travelTicketData.pet === '0'" class="value trustItem">
    ...
    <u-popup :show="showTips" round="16rpx">
      <view class="tipsBox">
        <view class="title">
          <u--text type="error" prefixIcon="warning" iconStyle="margin-right:20rpx;font-size: 40rpx;color:#f56c6c;" text="若确认修改出行信息, 您的座次可能会发生变化, 是否确认修改?" />
        </view>
        <view class="btns">
...
.ibg {
  ...
  background-image: linear-gradient(to bottom, #8b0001 500rpx, rgba(255,255,255,.0) 100%);
}
.ibgw {
  background: #fff;
  min-height: 100vh;
...
.travel {
  ...
  .travelBd {
    padding: 15rpx;
    background: url('/static/images/apply/cardBg.png') right bottom no-repeat;
...
.box {
  ...
  margin-top: 0;
  border-radius: 16rpx;
  background: url('/static/images/common/title1.jpg') top center no-repeat;
  background-size: cover;
  .hd {
    position: relative;
    padding: 20rpx;
