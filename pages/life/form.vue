<template>
  <view>
    ...
    <view class="head">
      <image src="/static/images/life/htitle.jpg" mode="widthFix"/>
    </view>

    <view class="main">
      <view class="box">
        <view class="hd">申请房屋地区</view>
        <view class="bd" @click="regionShow = true; hideKeyboard()">
          <view class="left">
            ...
            <view v-else class="pickerEmpty">请选择地区</view>
          </view>
          <view class="right">
            <u--text suffixIcon="arrow-right" align="right" size="28rpx" color="#2979ff" iconStyle="color: #2979ff;"
                     text="更改城市"/>
          </view>
        </view>
      </view>
      ...
      <view class="box">
        ...
        <view class="bd hx">
          ...
          <view v-for="item in houses" :key="item.id" class="item" :class="{ 'on': hxActive === item.id }"
                @click="changeHx(item)">
            <view class="inbox">
      ...
      <view v-if="hxActive" class="box">
        <view class="hd flex">已选户型
          <u--text type="error" bold :text="'( 赠送' + gfjjMoney + '万元生活保障金 )'" margin="0 0 0 20rpx"/>
        </view>
      ...
      <view v-if="hxActive > 3" class="box">
        ...
        <view class="bd car">
          ...
          <view v-for="(item, index) in cars" :key="item.id" class="item" :class="{ 'on': carActive === index }"
...
export default {
  data() {
    return {
      ...
      brand: '',
      typeShow: false,
      typeColumns: [[{value: 0, name: '本人申请'}, {value: 1, name: '委托申请'}], [{
        value: 0,
        name: '单独所有'
      }, {value: 1, name: '共同所有'}]],
  ...
  methods: {
    onSub() {
      ...
      let pData = {
        ...
        money: this.gfjjMoney,
        model: this.carId
      }
      // if (this.houseArea > 458) {
        // pData = this.carId
      // }
