<template>
	<view class="slide">
		<swiper :style="'height:'+heightNumber+'rpx'" :autoplay="autoplay" :interval="interval" :circular="circular"
			:current="swiperCurrent" :previous-margin="type==2?'30rpx':'0rpx'" :next-margin="type==2?'30rpx':'0rpx'"
			@change="handleChange">
			<swiper-item
				...
				<image :src="item.imgUrl" lazy-load :style="'border-radius:'+borderRadius+'rpx;'"
		...
		<view class="custom-indicator" v-if="indicatorType == 'dot'">
			<view v-for="(item,index) in list" :key="index"
				:class="['indicator-item',swiperCurrent==index?'indicator-item-active':'']"></view>
		</view>
		<view class="custom-number" v-if="indicatorType == 'num'">
			<view v-for="(item,index) in list" :key="index"
				class="indicator-item">{{ swiperCurrent + 1 }}/{{ list.length }}</view>
		</view>
	</view>
</template>
